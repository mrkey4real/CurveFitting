...
def main():
    ...
    print(f"")
    print(f"Very Conservative (0.1 kW): Keep {(power_data > 0.1).sum():,} points ({100*(power_data > 0.1).sum()/len(power_data):.1f}%)")
    print(f"  - Keeps most HVAC operation data")
    print(f"  - May include some standby/low power modes")
    print(f"")
    print(f"Conservative (0.2 kW): Keep {(power_data > 0.2).sum():,} points ({100*(power_data > 0.2).sum()/len(power_data):.1f}%)")
    print(f"  - Good balance of data quantity and quality")
    print(f"  - Recommended for initial analysis")
    print(f"")
    print(f"Moderate (0.4 kW): Keep {(power_data > 0.4).sum():,} points ({100*(power_data > 0.4).sum()/len(power_data):.1f}%)")
    print(f"  - Focuses on clear HVAC operation")
    print(f"  - Current threshold in comparison script")
    print(f"")
    print(f"Aggressive (0.8 kW): Keep {(power_data > 0.8).sum():,} points ({100*(power_data > 0.8).sum()/len(power_data):.1f}%)")
    print(f"  - High-power operation only")
    print(f"  - Good for studying peak performance")
    print(f"")
    print(f"Key insight: {100*(power_data == 0).sum()/len(power_data):.1f}% of data points are exactly 0 kW (HVAC off)")
    print(f"Median is 0, meaning >50% of time HVAC is off or very low power")
